...
func TestEmitExecAuditEvent(t *testing.T) {
	...
	mockServer, ok := scx.srv.(*mockServer)
	require.True(t, ok)

	mockEmitter, ok := mockServer.StreamEmitter.(*eventstest.MockEmitter)
	require.True(t, ok)
	...
	for _, tt := range tests {
		...
		execEvent := mockEmitter.LastEvent().(*apievents.Exec)
...
func TestContinue(t *testing.T) {
	...
	ctx := &ServerContext{
		ConnectionContext: &sshutils.ConnectionContext{
			ServerConn: &ssh.ServerConn{Conn: scx.ServerConn},
		},
		IsTestStub: true,
		srv:        scx.srv,
...
func newExecServerContext(t *testing.T) *ServerContext {
	scx := newTestServerContext(t, newMockServerWithBackend(t))
