...
// Multiple sessions are opened in parrallel tests to test for
...
func TestInteractiveSession(t *testing.T) {
	...
	// Don't run in parallel since the other tests will
	// cause clock.BlockUntil to block indefinitely.
	t.Run("LingerAndDie", func(t *testing.T) {
		sess := testOpenSession(t, reg)

		// Retrieve and close the party
		parties := sess.getParties()
		require.Equal(t, 1, len(parties))
		err := parties[0].Close()
		require.NoError(t, err)

		// Session should still be running with no parties
		require.False(t, sess.isStopped())

		// After the linger period, the session should be closed in the background
		clock.BlockUntil(2)
		clock.Advance(sess.lingerTTL)
		require.Eventually(t, sess.isStopped, time.Second*5, time.Millisecond*500)
	})

	t.Run("OpenJoinAndClose", func(t *testing.T) {
		t.Parallel()
		sess := testOpenSession(t, reg)
		...
		sess.Close()
		require.Empty(t, sess.getParties())
	})
